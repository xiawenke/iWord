...
while (1==1){
    ...
    if ($_GET[$i]==$_SESSION['key'][$i]){
        ...
        echo('<form action="wechat_dic.php" method="GET"><input type="hidden" name="word" value="'.$_SESSION["word"][$i].'"><button type="button" class="btn btn-default" onclick="this.form.submit()">去查查词典看看详细意思~</button></form>');
        echo '</div></div></div>';
...
if($_COOKIE['user']==""){
    echo('<div id="myAlert" class="alert alert-success">(提示：可以登录，把每一次测试的错词记录下来！)</div>');
    exit();
}
else{
    echo('<form action="wechat_note.php" method="POST"><input type="hidden" name="mark" value="'.$w_mark.'"><input type="hidden" name="word" value="'.$w_word.'"><button type="button" class="btn btn-default" onclick="this.form.submit()">将错词添加到单词本</button></form>');
}
